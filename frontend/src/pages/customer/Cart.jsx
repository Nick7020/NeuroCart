        ...
        <div className="lg:col-span-2 space-y-4">
          {items.map((item) => (
            <div key={item._id} className="card p-4 flex gap-4">
              <img
                src={item.product?.images?.[0] || 'https://placehold.co/80x80/1f2937/6366f1?text=P'}
                alt={item.product?.name}
                className="w-20 h-20 rounded-xl object-cover bg-gray-800 flex-shrink-0"
              />
              <div className="flex-1 min-w-0">
                <h3 className="font-semibold truncate">{item.product?.name}</h3>
                <p className="text-indigo-400 font-bold mt-1">{formatCurrency(item.price)}</p>
                ...
                <div className="flex items-center justify-between mt-3">
                  <div className="flex items-center gap-2 bg-gray-800 rounded-xl p-1">
                    <button
                      ...
                      className="w-7 h-7 flex items-center justify-center hover:bg-gray-700 rounded-lg transition-colors disabled:opacity-40"
                    >−</button>
                    <span className="w-6 text-center text-sm font-semibold">{item.quantity}</span>
                    <button
                      onClick={() => updateItem(item._id, item.quantity + 1)}
                      className="w-7 h-7 flex items-center justify-center hover:bg-gray-700 rounded-lg transition-colors"
                    >+</button>
                  </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button
                      onClick={async () => { await removeItem(item._id); notify('Removed from cart', 'info') }}
